...
/** Normalise a notification: handle both old (read/message/link) and new (is_read/body/related_id) DB schemas */
...
/** Navigate to relevant page based on link field or content heuristics */
...
const TYPE_CONFIG: Record<string, { icon: typeof Info; color: string; bg: string; emoji: string }> = {
  task_assigned:  { icon: CheckCircle,   color: '#3B82F6', bg: 'rgba(59,130,246,0.1)',   emoji: '📌' },
  task_completed: { icon: CheckCircle,   color: '#10B981', bg: 'rgba(16,185,129,0.1)',   emoji: '✅' },
  task_shared:    { icon: Info,          color: '#6366F1', bg: 'rgba(99,102,241,0.1)',   emoji: '📝' },
  access_granted: { icon: CheckCircle,   color: '#8B5CF6', bg: 'rgba(139,92,246,0.1)',  emoji: '🔓' },
  team_update:    { icon: AlertTriangle, color: '#F97316', bg: 'rgba(249,115,22,0.1)',   emoji: '👥' },
  message:        { icon: CheckCircle,   color: '#10B981', bg: 'rgba(16,185,129,0.1)',   emoji: '💬' },
  reply:          { icon: Info,          color: '#3B82F6', bg: 'rgba(59,130,246,0.1)',   emoji: '↩️' },
  success:        { icon: CheckCircle,   color: '#10B981', bg: 'rgba(16,185,129,0.1)',   emoji: '✅' },
  warning:        { icon: AlertTriangle, color: '#F59E0B', bg: 'rgba(245,158,11,0.1)',   emoji: '⚠️' },
  error:          { icon: XCircle,       color: '#EF4444', bg: 'rgba(239,68,68,0.1)',    emoji: '❌' },
  info:           { icon: Info,          color: '#3B82F6', bg: 'rgba(59,130,246,0.1)',   emoji: '🔔' },
}
...
export function NotificationPanel({ initialCount = 0, currentUsername = '' }: NotificationPanelProps) {
  ...
  async function handleNotifClick(notif: Notification) {
    ...
    if (!n.isRead) {
      ...
      if (res.success) {
        setNotifications(prev => prev.map(x => x.id === notif.id ? { ...x, read: true, is_read: true } : x))
        syncUnreadFromList(notifications.map(x => x.id === notif.id ? { ...x, read: true } : x))
      }
    }
    setOpen(false)
                    ...
                    <div key={notif.id}>
                      ...
                      >
                        ...
                        <button
                          ...
                          className="w-full flex items-start gap-3 px-5 py-3.5 text-left pr-16"
                        >
                          {/* Icon */}
                          ...
                          <div className="px-5 pb-3 pt-0 pl-17" onClick={e => e.stopPropagation()}>
                            <div className="flex gap-2 items-center">
                              <input
                                ...
                                onKeyDown={e => { if (e.key === 'Enter') handleSendReply(notif); if (e.key === 'Escape') setReplyingTo(null) }}

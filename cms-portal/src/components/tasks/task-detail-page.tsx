...
function fmtTs(ts: string) {
  try {
    return format(new Date(ts), "MMM d, yyyy 'at' hh:mm aa")
  } catch {
    return ts
  }
}
      ...
      <div className="min-h-full bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.08),_transparent_30%),linear-gradient(180deg,#f8fbff_0%,#eef4fb_100%)] px-4 py-5 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-[1600px] flex-col gap-5">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-[28px] border border-white/80 bg-white/85 px-5 py-4 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur">
            <div className="min-w-0">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-500">
                ...
                <span className="flex items-center gap-1.5"><Clock size={14} /> {format(new Date(t.created_at), 'MMM d, yyyy')}</span>
                {t.due_date && (
          ...
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_380px]">
                ...
                {activeTab === 'info' && (
                  <div className="space-y-6">
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                      ...
                      <MetaCard icon={<Calendar size={13} className="text-orange-500" />} label="Due Date" value={t.due_date ? format(new Date(t.due_date), 'MMM d, yyyy') : '-'} accent={!isCompleted && t.due_date && new Date(t.due_date) < new Date() ? 'red' : undefined} />
                            ...
                            <div key={attachment.id} className="flex flex-wrap items-center gap-3 rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-3">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <p className="text-xs text-slate-400">
                                  {attachment.uploaded_by} · {format(new Date(attachment.created_at), 'MMM d, yyyy')}
            ...
            <aside className="flex min-h-[720px] flex-col overflow-hidden rounded-[32px] border border-white/80 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
              ...
              <div className="border-b border-slate-100 px-5 py-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {participants.map((participant) => (
                    <div key={participant.username} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                      <p className="text-sm font-semibold text-slate-800">{participant.username}</p>
                    ...
                    return (
                      <div key={`${entry.timestamp}-${index}`} className={cn('flex gap-3', isMe && 'justify-end')}>
                        ...
                        <div className={cn('max-w-[85%]', isMe && 'items-end')}>
                          ...
                          <div className={cn('mt-1 px-1 text-[11px] text-slate-400', isMe && 'text-right')}>
